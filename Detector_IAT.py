...
def get_iat_features(path,hasher):
    iat_features = {}
    # extract iat from binary file
    try:
        pe = pefile.PE(path)
        pe.parse_data_directories()
        # store iat features in dictionary form
        try:
            ...
            print("Error: Unable to parse {0}".format(path))
    except:
        print("Error: Unable to open file {0}".format(path))
    
    # convert iat features to numpy array
    #print(function.name)
    ...
    hashed_features = numpy.asarray(hashed_features)[0]
    #print("Finished Extract IAT Feature")
    # return hashed iat features
    print("Extracted {0} iat from {1}".format(len(iat_features),path))
    return hashed_features

def scan_file(path):
    # scan a file to determine if it is malicious or benign
    if not os.path.exists("saved_detector_IAT.pkl"):
        print("It appears you haven't trained a detector yet!  Do this before scanning files.")
        sys.exit(1)
    with open("saved_detector_IAT.pkl","rb") as saved_detector:
...
def train_detector(benign_path,malicious_path,hasher):
    ...
    pickle.dump((classifier,hasher),open("saved_detector_IAT.pkl","wb+"))
